...
void RingBuffer_state(/* CAN transmitter function  */);    
  // SD Functions
void sdConfig();
void sdSave();
String packetToString(bool err = true);
int countFiles(File dir);
void canFilter(/* CAN receiver function */);   
...
void setup()
{
  ...
  CAN_cfg.rx_queue = xQueueCreate(rx_queue_size*4, sizeof(CAN_frame_t)); // Create a queue for data receive

  if(CAN_cfg.rx_queue == NULL)
  {
    Serial.println("Queue creator ERROR");
    return;
  }
  ...
  txMsg.FIR.B.FF = CAN_frame_std;
  txMsg.FIR.B.DLC = 8;

  setupVolatilePacket(); // volatile packet default values
  taskSetup();           // Tasks

  timeoutSOT.attach(1.0, debouceHandlerSOT);
  ticker40Hz.attach(1/40.0, ticker40HzISR);
}
...
void RingBuffer_state()
{
  ...
  Serial.printf("%s\r\n", current_state==SOT_ST ? "SOT state" : "...");
